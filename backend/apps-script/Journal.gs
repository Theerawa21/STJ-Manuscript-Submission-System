...
var JOURNAL_SCHEMA_ = {
  ...
  REMINDER_LOG:['assignment_id','reminder_type','due_date','sent_at','queued_at'],
...
function setupJournal(q){return response_(function(){
  ...
  var lock=LockService.getScriptLock();lock.waitLock(30000);try{
    ...
    jAppend_('MIGRATIONS',{migration_id:jUuid_(),executed_at:jNow_(),backup_id:backup.getId(),schema_version:'2'});if(q.logoFile){var lf=jSaveFile_(null,{id:'EDITOR_SHARED'},q.logoFile,'COVER',1,'EDITOR');p.setProperty('JOURNAL_LOGO_FILE_ID',jFind_('FILES','file_id',lf).storage_id);}rows_('SUBMISSIONS').forEach(jRegisterOriginals_);
    ...
    p.setProperty('JOURNAL_READY','true');jAudit_({id:'EDITOR_SHARED',role:'EDITOR'},'INITIALIZE_JOURNAL','','','schema 2');return {ready:true,backupId:backup.getId(),fileBackupId:storageBackup.getId()};
  }finally{lock.releaseLock();}
});}
...
function grantStaffRole(q){return jWrite_(q,'GRANT_ROLE','EDITOR',function(actor,tx){if(actor.id!=='EDITOR_SHARED'&&!jRows_('USER_ROLES').some(function(r){return r.user_id===actor.id&&r.role==='ADMIN'&&String(r.active)==='true';}))throw userError_('เฉพาะผู้ดูแลระบบเท่านั้นที่กำหนดบทบาทได้');if(['EDITOR','MANAGING_EDITOR','PROOFREADER','ADMIN','AUTHOR'].indexOf(q.role)<0||!rows_('USERS').some(function(u){return u.user_id===q.userId;}))throw userError_('บัญชีหรือบทบาทไม่ถูกต้อง');if(q.manuscriptId&&q.role==='PROOFREADER'&&!jMeta_(jManuscript_(q.manuscriptId)).accepted_at)throw userError_('มอบหมายตรวจภาษาได้เฉพาะบทความตอบรับ');tx.start();var r=jRows_('USER_ROLES').find(function(r){return r.user_id===q.userId&&r.role===q.role;}),data={user_id:q.userId,role:q.role,active:q.active!==false&&q.active!=='false'};if(r)jUpdate_('USER_ROLES',r._row,data);else jAppend_('USER_ROLES',data);if(q.manuscriptId&&q.role==='PROOFREADER'){var s=jManuscript_(q.manuscriptId);if(!jMeta_(s).accepted_at)throw userError_('มอบหมายตรวจภาษาได้เฉพาะบทความตอบรับ');jAppend_('PRODUCTION_ASSIGNMENTS',{manuscript_id:s.manuscript_id,user_id:q.userId,assigned_at:jNow_()});}return {saved:true};});}
